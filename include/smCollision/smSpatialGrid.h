...
//#define SOFMIS_SPATIALGRID_MAXIMUMLATTICELIST   100
...
class smSpatialGridWorker:public smWorkerThread{
	//smSceneObject *objectList[SOFMIS_SPATIALGRID_MAXIMUMLATTICELIST];
    //smInt objectCounter;
...
public:
	...
	inline void checkNarrow(smLattice *p_latticeA, smLattice *p_latticeB, smInt p_primAIndex,smInt p_primBIndex){
		 ...
		 if(smCollisionUtils::tri2tri(p_latticeA->mesh->vertices[p_latticeA->mesh->triangles[p_primAIndex].vert[0]],
								   ...
								   projectedPoint2)){
									   ...
									   //pairs[collidedPairs].objectIndex=p_latticeA->linkedObject;
									   //pairs[collidedPairs].objectIndex2=p_latticeB->linkedObject;
									   //pairs[collidedPairs].primIndex=p_primAIndex;
									   //pairs[collidedPairs].primIndex2=p_primBIndex;
	...
	virtual void kernel(){
			collidedPairs=0;
			/*for(smInt i=0;i<objectCounter;i++){
				
				objectList[i]->memBlock->getBlock(QString("lattice"),(void**)&latticePair);
				// lat->test->get

				
				latticePair->indexReset();
				latticePair->updateBounds();

				
			}  
				for(smInt i=0;i<objectCounter;i++){
					objectList[i]->memBlock->getBlock(QString("lattice"),(void**)&latticePair);
					for(smInt j=i+1;j<objectCounter;j++)
					{  
						objectList[j]->memBlock->getBlock(QString("lattice"),(void**)&latticePair2);
						for(smInt cellIndex=0;cellIndex<latticePair->totalCells;cellIndex++){
							//no primitives inside the cell
							if(latticePair2->cells[cellIndex].lastPrimitiveIndex==0)
								continue;
		   ...
		   //cout<<"My id x="<<id.x<<" "<<id.y<<" "<<id.z<<endl; 
		   //cout<<"Collided Pairs:"<<collidedPairs<<endl;
...
public:
	...
	void init(){
		...
		/*for (smInt i=0;i<totalLattices; i++){
			latticeList[i]->init(SOFMIS_SPATIALGRID_LEFTCORNER,SOFMIS_SPATIALGRID_RIGHTCORNER,
		...
		*/
		...
		for(smInt i=0;i<sceneList.size();i++){
			...
			for(smInt j=0;j<scene->getTotalObjects();j++){
				sceneObject=scene->sceneObjects.at(j);
				objectType=sceneObject->getType();
				switch(objectType){
						case SOFMIS_SMSTATICSCENEOBJECT:
							staticSceneObj=(smStaticSceneObject*)sceneObject;
							staticSceneObj->memBlock->allocate<smLattice>(QString("lattice"),&lat);
							lat->init(leftCorner,rightCorner,xSeperation,ySeperation,zSeperation);
						break;
				}//end of switch
			}//end of scene objects
		}//end of scenes traverse	
		*/
		isInitialized=true;
	...
	void run(){
		...
		if(isInitialized==false)
			return;
		startWorkers();
		//cout<<"Workers thread started..."<<endl;
		while(true&&this->terminateExecution==false){
			beginModule();
			nbrTriTriCollisions=0;

			//cout<<"Workers are about to start..."<<endl;
	...
	//virtual void draw(void *p_params){
		for(smInt i=0;i<totalThreads;i++){
			workerThreads[i].draw();
		}
