...
****************************************************
    ...
    \date       04/2009
    \bug	    None yet
    \brief	    This Module automates the memory allocation 
...
class smMemoryBlock:public smCoreClass{
	...
	public:
	   ...
	   ///deletes the block from memeory as well as in has container
		...
		/*template<class T>
		smMemReturnType originaltoLocalBlock(T* dst,T* src, smInt p_nbr){
		...
		template<class T>
		smMemReturnType originaltoLocalBlock(const QString &p_memoryBlockName, T *src, const smInt p_nbr)
		{
			T *dst;
			if(src!=NULL){
