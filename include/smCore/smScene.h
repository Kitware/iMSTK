...
public:
    ...
    void handleEvents(std::shared_ptr<smtk::Event::smEvent> event)
    {
...
//                         sdk->shutDown();
                    ...
                    case smtk::Event::smKey::W:
                    {
                        ...
                        {
                            //Move the camera up
                            camera->setCameraPos(camera->pos.x, camera->pos.y + 1, camera->pos.z);
                            camera->setCameraFocus(camera->fp.x, camera->fp.y + 1, camera->fp.z);
                            camera->genViewMat();
                        }
                        else
                        {
                            //Move the camera forward
                            camera->setCameraPos(camera->pos.x, camera->pos.y, camera->pos.z - 1);
                            camera->setCameraFocus(camera->fp.x, camera->fp.y, camera->fp.z - 1);
                            camera->genViewMat();
                        }
                        break;
                    }
                    case smtk::Event::smKey::A:
                    {
                        if(smtk::Event::smModKey::shift == (keyboardEvent->getModifierKey() & smtk::Event::smModKey::shift))
                        {
                            ...
                            camera->setCameraPos(camera->pos.x - 1, camera->pos.y, camera->pos.z);
                            camera->setCameraFocus(camera->fp.x - 1, camera->fp.y, camera->fp.z);
                            camera->genViewMat();
                        }
        ...
        auto mouseButtonEvent = std::static_pointer_cast<smtk::Event::smMouseButtonEvent>(event);
        if(mouseButtonEvent != nullptr)
        {
            switch(mouseButtonEvent->getMouseButton())
            {
                case smMouseButton::Left:
                {
                    std::cout << "Left" << std::endl;

                    break;
                }
                case smMouseButton::Right:
                {
                    std::cout << "Right" << std::endl;

                    break;
                }
                case smMouseButton::Middle:
                {
                ...
                default:
                {
                    std::cout << " x: " << mouseButtonEvent->getWindowCoord()[0] << " y: " << mouseButtonEvent->getWindowCoord()[1] << "\n";
                }
            }
            return;
        }

        auto mouseMoveEvent = std::static_pointer_cast<smtk::Event::smMouseMoveEvent>(event);
        if(mouseMoveEvent != nullptr)
        {
            smVec2d coord = mouseMoveEvent->getWindowCoord();
            std::cout << "mpData: x: " << coord[0] << " y: " << coord[1] << std::endl;
        }
