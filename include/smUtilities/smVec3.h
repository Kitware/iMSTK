...
********************************************************************
    ...
    \date       04/2009
    \bug	    None yet
    \brief	    This Module is for general Vector class. It provides several has vector operations.
    \brief      Todo list:
                rotate function 
...
public:
   ...
   inline  smVec3 unit()
	...
	/// Transform the vector with a transformation matrix

	/*inline smVec3 &operator *(const smMatrix44<T> t_m) {

		smVec3 vv;

		vv.x= x*t_m[0][0]+y*t_m[0][1]+z*t_m[0][2];
		vv.y= x*t_m[1][0]+y*t_m[1][1]+z*t_m[1][2];
		vv.z= x*t_m[2][0]+y*t_m[2][1]+z*t_m[2][2];
 ...
 /*  static const smVec3<smFloat> UNITX;
   static const smVec3<smFloat> UNITY;
   static const smVec3<smFloat> UNITZ;
 */  
