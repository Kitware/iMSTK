...
\date       05/2009
\bug	    None yet
\brief	    this class is for collision detection for the scene objects.
*****************************************************
*/
...
public: 
	...
	//smPrimConnector<smPrimitive,class meshType>* primConnector;
	smCell *cells;
	smInt totalCells;
	smInt xSeperation;
	smInt ySeperation;
	...
	//smLatticeReturnType init(smVec3 p_leftCorner,smVec3 rightCorner, smInt xSeperation, smInt ySeperation, smInt zSeperation );
	//template <class smPrimitive>
	smLatticeReturnType init(smVec3<smFloat> p_leftCorner,smVec3<smFloat> p_rightCorner, smInt p_xSeperation, smInt p_ySeperation, smInt p_zSeperation ){
		smInt x,y,z;
		smInt index;
	...
	inline virtual void  linkPrimitivetoCell(smInt p_primitiveIndex){
		//smLattice<smTrianglePrimitive,smSurfaceMesh> *lattice;
		smInt minX;
		smInt minY;
		smInt minZ;
   		smInt maxX;
		...
		//lattice=(smLattice<smTrianglePrimitive,smSurfaceMesh>*)p_lattice;
	...
	inline void updateBounds(smSurfaceMesh* p_mesh,smInt p_index){
	     //min
		aabb[p_index].aabbMin.x=  SOFMIS_MIN(p_mesh->vertices[p_mesh->triangles[p_index].vert[0]].x,p_mesh->vertices[p_mesh->triangles[p_index].vert[1]].x);
		aabb[p_index].aabbMin.x = SOFMIS_MIN(aabb[p_index].aabbMin.x,p_mesh->vertices[p_mesh->triangles[p_index].vert[2]].x); 

		aabb[p_index].aabbMin.y=  SOFMIS_MIN(p_mesh->vertices[p_mesh->triangles[p_index].vert[0]].y,p_mesh->vertices[p_mesh->triangles[p_index].vert[1]].y);
		aabb[p_index].aabbMin.y = SOFMIS_MIN(aabb[p_index].aabbMin.y,p_mesh->vertices[p_mesh->triangles[p_index].vert[2]].y); 

		aabb[p_index].aabbMin.z=  SOFMIS_MIN(p_mesh->vertices[p_mesh->triangles[p_index].vert[0]].z,p_mesh->vertices[p_mesh->triangles[p_index].vert[1]].z);
		aabb[p_index].aabbMin.z = SOFMIS_MIN(aabb[p_index].aabbMin.z,p_mesh->vertices[p_mesh->triangles[p_index].vert[2]].z); 
        
        //max
		aabb[p_index].aabbMax.x=  SOFMIS_MAX(p_mesh->vertices[p_mesh->triangles[p_index].vert[0]].x,p_mesh->vertices[p_mesh->triangles[p_index].vert[1]].x);
		aabb[p_index].aabbMax.x = SOFMIS_MAX(aabb[p_index].aabbMax.x,p_mesh->vertices[p_mesh->triangles[p_index].vert[2]].x); 

		aabb[p_index].aabbMax.y=  SOFMIS_MAX(p_mesh->vertices[p_mesh->triangles[p_index].vert[0]].y,p_mesh->vertices[p_mesh->triangles[p_index].vert[1]].y);
		aabb[p_index].aabbMax.y = SOFMIS_MAX(aabb[p_index].aabbMax.y,p_mesh->vertices[p_mesh->triangles[p_index].vert[2]].y); 

		aabb[p_index].aabbMax.z=  SOFMIS_MAX(p_mesh->vertices[p_mesh->triangles[p_index].vert[0]].z,p_mesh->vertices[p_mesh->triangles[p_index].vert[1]].z);
		aabb[p_index].aabbMax.z = SOFMIS_MAX(aabb[p_index].aabbMax.z,p_mesh->vertices[p_mesh->triangles[p_index].vert[2]].z); 
	...
	void updateBounds(){
		...
		for(smInt i=0;i<mesh->nbrTriangles;i++)
		  updateBounds(mesh,i);		
		//smPrimitive::updateBounds(mesh,primitives);
	}
    ...
    {	for(int j=0;j<ySeperation;j++){
        ...
        for(int i=0;i<xSeperation;i++){
            ...
            glVertex3f( cells[index].cellLeftCorner[0], cells[index].cellLeftCorner[1], cells[index].cellLeftCorner[2]-4*zStep);
            glVertex3f(cells[index2].cellLeftCorner[0],cells[index2].cellLeftCorner[1],cells[index2].cellLeftCorner[2]+4*zStep);
        ...
        for(int i=0;i<zSeperation;i++){
            ...
            glVertex3f( cells[index].cellLeftCorner[0]-4*xStep, cells[index].cellLeftCorner[1], cells[index].cellLeftCorner[2]);
            glVertex3f(cells[index2].cellLeftCorner[0]+4*xStep,cells[index2].cellLeftCorner[1],cells[index2].cellLeftCorner[2]);
            ...
            for(int z=0;z<zSeperation;z++)
                for(int x=0;x<xSeperation;x++){
					...
					if(latticeMode&SOFMIS_SMLATTICE_CELLPOINTSLINKS){
                        ...
                        glVertex3f(cells[index].cellLeftCorner[0]+xStep,cells[index].cellLeftCorner[1],cells[index].cellLeftCorner[2]);
                        glVertex3f(cells[index].cellLeftCorner[0]+xStep,cells[index].cellLeftCorner[1],cells[index].cellLeftCorner[2]+zStep);
                        glVertex3f(cells[index].cellLeftCorner[0],cells[index].cellLeftCorner[1],cells[index].cellLeftCorner[2]+zStep);
                        glVertex3fv((GLfloat*)&cells[index].cellLeftCorner);

                        glVertex3f(cells[index].cellLeftCorner[0],cells[index].cellLeftCorner[1]+yStep,cells[index].cellLeftCorner[2]);
                        glVertex3f(cells[index].cellLeftCorner[0]+xStep,cells[index].cellLeftCorner[1]+yStep,cells[index].cellLeftCorner[2]);
                        glVertex3f(cells[index].cellLeftCorner[0]+xStep,cells[index].cellLeftCorner[1]+yStep,cells[index].cellLeftCorner[2]+zStep);
                        glVertex3f(cells[index].cellLeftCorner[0],cells[index].cellLeftCorner[1]+yStep,cells[index].cellLeftCorner[2]+zStep);
                        glVertex3f(cells[index].cellLeftCorner[0],cells[index].cellLeftCorner[1]+yStep,cells[index].cellLeftCorner[2]);
                        glEnd();
                        glBegin(GL_LINES);
						glColor3fv((GLfloat*)&smColor::colorWhite);
    ...
    if(latticeMode&SOFMIS_SMLATTICE_MINMAXPOINTS){
        ...
        glPopMatrix();
        glPushMatrix();
        glTranslatef(cells[this->totalCells-1].cellRightCorner[0],cells[this->totalCells-1].cellRightCorner[1],cells[this->totalCells-1].cellRightCorner[2]);
