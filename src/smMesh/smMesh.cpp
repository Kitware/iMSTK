...
void CalculateTangentArray(smInt vertexCount, const smVec3f *vertex, const smVec3f *normal,
	const smTexCoord *texcoord, long triangleCount, const smTriangle *triangle, smVec3f *tangent)
{
 ...
 void smMesh::updateVertexNormals()
{
	...
	for(smInt i=0;i<nbrVertices; i++)
	{
		...
		//vertNormals[i] = temp/(vertTriNeighbors[i].size());
		vertNormals[i]=temp;
...
{		
	...
	/*for(i=0; i<nbrVertices; i++)		
		qSort(vertTriNeighbors[i].begin(),vertTriNeighbors[i].end());*/
	...
	for(i=0; i<nbrVertices; i++)
	{
		for(j=0;j<vertVertNeighbors[i].size();j++)
		{
			...
			if(vertVertNeighbors[i][j]>i){
				...
				//	if(edges.at(k).vert[0] == i && edges.at(k).vert[1] == neighborsVertices[i].at(j)) exist = true;
				//	if(edges.at(k).vert[1] == i && edges.at(k).vert[0] == neighborsVertices[i].at(j)) exist = true;				
				//}
				//if(!exist){
					edge.vert[0]=i;
...
void smMesh::draw(smDrawParam p_params){
	...
	/*if(viewer->renderStage==SMRENDERSTAGE_SHADOWPASS&&(p_params.caller->renderDetail.shaderEnable==false))
		p_params.caller->renderDetail.shaderEnable=true;
	else
		p_params.caller->renderDetail.shaderEnable=false;
		*/
  ...
  for(smInt i=0;i<nbrTriangles;i++)
	{
		...
		triAABBs[i].aabbMax.z = SOFMIS_MAX(vertices[triangles[i].vert[0]].z, vertices[triangles[i].vert[1]].z); 
