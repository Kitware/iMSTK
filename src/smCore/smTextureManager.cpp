...
smTextureReturnType smTextureManager::initGLTextures(){
		...
		for(smInt i=0;i<textures.size();i++){
			...
			/*if(data.imageColorType==SOFMIS_IMAGECOLOR_RGBA)
				...
				gluBuild2DMipmaps( GL_TEXTURE_2D,GL_RGB, ilGetInteger(IL_IMAGE_WIDTH), ilGetInteger(IL_IMAGE_HEIGHT),
...
///load the texture and associated it with reference name. Also you could use texture Id for activating
	///the texture
smTextureReturnType smTextureManager::loadTexture(const smChar *p_fileName,const smChar *p_textureReferenceName,smInt &p_textureId){
		smTexture *texture;		
		ILenum error;
		ILuint imageName;
...
smTextureReturnType smTextureManager::loadTexture(const smString p_fileName, const smChar *p_textureReferenceName,smBool p_flipImage){
		//smChar *fileName;
		//fileName=
...
///if the texture is not loaded previously, create and the texture return the id
smTextureReturnType smTextureManager::findTextureId(const smChar *p_textureReferenceName, smInt &p_textureId){
		smInt textureId;
		smTexture *texture;
...
///This function binds the texture to the appropriate texture.
...
///This function binds the texture to the appropriate shader  texture.
///first parameter is texture reference
///For instance if the argument is 0, the it will bind to GL_TEXTURE0
///Also for the shader the binded name will be p_shaderBindName
GLuint smTextureManager::activateTexture(const smChar *p_textureReferenceName,smInt p_textureGLOrder,smInt p_shaderBindGLId){
		smInt textureId;
		smTexture *texture;
		textureId=textureIndexId[p_textureReferenceName];
		texture=textures[textureId];
...
///This function binds the smTexture to the appropriate shader texture.
///For instance if the argument is 0, the it will bind to GL_TEXTURE0
///Also for the shader the binded name will be p_shaderBindName
GLuint smTextureManager::activateTexture(smTexture *p_texture,smInt p_textureGLOrder,smInt p_shaderBindGLId){
...
void smTextureManager::initDepthTexture(smTexture *p_texture){
 ...
 ////   //glTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP, GL_TRUE);
	...
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);//original
    //glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);//original
	...
	//glTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP, GL_TRUE);
	//gluBuild2DMipmaps( GL_TEXTURE_2D, 3, p_texture->width, p_texture->height,GL_RGB, GL_UNSIGNED_BYTE,0 );
...
void smTextureManager::initColorTexture(smTexture *p_texture){
	...
	glTexImage2D( GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, p_texture->width, p_texture->height, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE, 0);
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP );
	glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP );
    //glTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP, GL_TRUE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
