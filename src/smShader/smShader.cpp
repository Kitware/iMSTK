...
smBool smShader::initShaders(smChar *p_vertexProgFileName,smChar *p_fragmentProgFileName,smChar *p_geometryProgFileName){
    ...
    if(p_vertexProgFileName!=NULL&&strlen(p_vertexProgFileName)>0){
        ...
        if(vertexShaderFile){
            ...
            //vertexShaderContent[fileSize]='\0';
            //cout<<vertexShaderContent;
    ...
    /*GLint ok = false;
    glGetObjectParameterivARB(shaderProgramObject, GL_OBJECT_LINK_STATUS_ARB, &ok);
    if (!ok)
    {
      int maxLength=4096;
      char *infoLog = new char[maxLength];
      glGetInfoLogARB(shaderProgramObject, maxLength, &maxLength, infoLog);
...
void smShader::enableShader(){
    ...
    if (vertexProgramExist)		glEnable(GL_VERTEX_PROGRAM_ARB);
	if(fragmentProgramExist)	glEnable(GL_FRAGMENT_PROGRAM_ARB);
	if(geometryProgramExist)	glEnable(GL_GEOMETRY_SHADER_ARB);
...
#ifdef SOFMIS_OPENGL_SHADER
     if (vertexProgramExist)	 glDisable(GL_VERTEX_PROGRAM_ARB);
     if(fragmentProgramExist) glDisable(GL_FRAGMENT_PROGRAM_ARB);
	 if(geometryProgramExist) glDisable(GL_GEOMETRY_SHADER_ARB);
...
void smShader::saveAndDisableCurrent(){
	...
	if(currentShader!=NULL){
	 if (smShader::currentShader->vertexProgramExist)	glDisable(GL_VERTEX_PROGRAM_ARB);
     if(smShader::currentShader->fragmentProgramExist) glDisable(GL_FRAGMENT_PROGRAM_ARB);
	 if(smShader::currentShader->geometryProgramExist) glDisable(GL_GEOMETRY_SHADER_ARB);
	 smShader::currentShaderEnabled=false;
...
smBool smShader::attachTexture(smUnifiedID p_meshID, smChar * const p_textureName, smChar * const p_textureShaderName){
	smTextureShaderAssignment assign;
...
void smShader::autoGetTextureIds(){
	//for(smInt i=0;i<textureGLBind.size();i++)
	
	QMultiHash<smInt,smTextureShaderAssignment>::iterator i=texAssignments.begin() ;
	//cout<<"[smShader::autoGetTextureIds] Auto Assignment Begin"<<endl;
	for(;i!=texAssignments.end();i++){
		
		i.value().textureShaderGLassignment=textureGLBind[i.value().shaderParamName];
		//cout<<"[smShader::autoGetTextureIds] textureShaderGLassignment:"<<i.value().textureShaderGLassignment<<" shaderParamName"<<i.value().shaderParamName.toAscii().data()<<" "<<i.value().textureId   <<  endl;
	}
	//textureGLBind
	

	/*
	cout<<"[smShader::autoGetTextureIds] Auto Assignment End"<<endl;
	i=texAssignments.begin() ;
	cout<<"[smShader::autoGetTextureIds] Double Check Begin"<<endl;
	for(;i!=texAssignments.end();i++){
		
		
		cout<<"[smShader::autoGetTextureIds] "<<i.value().textureShaderGLassignment<<" "<<i.value().shaderParamName.toAscii().data()<<endl;
	}
	cout<<"[smShader::autoGetTextureIds] Double Check End"<<endl;*/
...
smBool smShader::setShaderFileName(smChar *p_vertexFileName,smChar *p_geometryFileName, smChar *p_fragmentFileName)
{
...
void smShader::activeGLTextures(smUnifiedID p_id){
	...
	for(smInt i=0;i<values.size();i++){
		//cout<<"[smShader]  paramName:"<<values[i].shaderParamName.toAscii().data();
		//cout<<" textureShaderGLassignment:"<<values[i].textureShaderGLassignment;
		//cout<<" textureId:"<<values[i].textureId<<endl;
...
void smShader::print(){
	for(smInt i=0;i<vertexShaderParamsString.size();i++){
		//cout<<"Param:"<<vertexShaderParamsString[i]<<" GL locations:"<<vertexShaderParams[i]<<endl;
