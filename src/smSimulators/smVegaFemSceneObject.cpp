...
smVegaFemSceneObject::~smVegaFemSceneObject(){
	...
	if (u != NULL) free(u);
	if (uvel != NULL) free(uvel);
	if (uaccel != NULL) free(uaccel);
	if (f_ext != NULL) free(f_ext);
	if (f_extBase != NULL) free(f_extBase);
	if (uSecondary != NULL) free(uSecondary);
	if (uInitial != NULL) free(uInitial);
	if (velInitial != NULL) free(velInitial);

	//REVISIT!
	if (fixedVertices != NULL) delete fixedVertices;
	if (integratorBase != NULL) delete integratorBase;
	if (implicitNewmarkSparse != NULL) delete implicitNewmarkSparse;
	if (integratorBaseSparse != NULL) delete integratorBaseSparse;
	if (forceModel != NULL) delete forceModel;
	if (stVKInternalForces != NULL) delete stVKInternalForces;
	if (stVKStiffnessMatrix != NULL) delete stVKStiffnessMatrix;
	if (stVKForceModel != NULL) delete stVKForceModel;
	if (massSpringSystemForceModel != NULL) delete massSpringSystemForceModel;
	if (corotationalLinearFEMForceModel != NULL) delete corotationalLinearFEMForceModel;
	if (volumetricMesh != NULL) delete volumetricMesh;
	if (tetMesh != NULL) delete tetMesh;
	if (meshGraph != NULL) delete meshGraph;
	if (massSpringSystem != NULL) delete massSpringSystem;
	if (renderMassSprings != NULL) delete renderMassSprings;
	if (massMatrix != NULL) delete massMatrix;
	if (LaplacianDampingMatrix != NULL) delete LaplacianDampingMatrix;
