...
void smFemSceneObject::computeStiffness(){
	...
	FILE* check1;
	FILE* check2;
	if((check1 = fopen("models/Floppy_K.dat","rb"))&&(check2 = fopen("models/Floppy_Kinv.dat","rb"))){
	...
	else
	{
		...
		for(i=0; i<v_mesh->nbrTetra; i++){
			...
			}coff(3,0)=1.0;coff(3,1)=1.0;coff(3,2)=1.0;coff(3,3)=1.0;
	...
	if(dynamicFem){
		...
		lumpMasses();
		//nodeMass=0.2*VectorXf::Ones(v_mesh->nbrNodes);
...
void smFemSceneObject::lumpMasses(){
	...
	for(i=0;i<v_mesh->nbrTetra;i++){
		...
		node0(0) = v_mesh->nodes[0].x; node0(1) = v_mesh->nodes[0].y; node0(2) = v_mesh->nodes[0].z;
		node1(0) = v_mesh->nodes[1].x; node1(1) = v_mesh->nodes[1].y; node1(2) = v_mesh->nodes[1].z;
		node2(0) = v_mesh->nodes[2].x; node2(1) = v_mesh->nodes[2].y; node2(2) = v_mesh->nodes[2].z;
		node3(0) = v_mesh->nodes[3].x; node3(1) = v_mesh->nodes[3].y; node3(2) = v_mesh->nodes[3].z;
...
void smFemSceneObject::calculateDisplacements_QStatic(smVec3<smFloat> *vertices){
	...
	//if(displacements(dofNumber,0)>=3.0)
	//	pullUp=false;		
	//else if(displacements(dofNumber,0)<=0.0)
	//	pullUp=true;

	//if(pullUp)
	//	displacements(dofNumber,0)+=0.04;	
	//else
	//	displacements(dofNumber,0)-=0.04;
