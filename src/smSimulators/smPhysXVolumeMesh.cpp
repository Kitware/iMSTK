...
smPhysXVolumeMesh::~smPhysXVolumeMesh()
{
	if(neiTet!=NULL) {
		for(smInt i=0; this->nbrNodes; i++){
			if(this->neiTet[i].idx!=NULL) delete [] this->neiTet[i].idx;
		}
		delete [] neiTet;
	}	
}
...

{
	...
	for(smInt i=0;i<this->nbrTetra;i++) this->drawTet[i] = true;
	return true;
}
...
void smPhysXVolumeMesh::draw(smDrawParam p_params){
	...
	glBegin(GL_POINTS);
	for(smInt i=0;i<this->nbrNodes;i++){
		glVertex3fv((GLfloat*)&this->nodes[i]);
	...
	*/
	...
	if(renderTetras){
			...
			for(smInt i = 0; i < this->nbrTetra; i++)
			{
				...
				if(this->drawTet[i]){		
					/*
					smVec3f p0(this->nodes[this->tetra[i].vert[0]]);
					smVec3f p1(this->nodes[this->tetra[i].vert[1]]);
					smVec3f p2(this->nodes[this->tetra[i].vert[2]]);
					smVec3f p3(this->nodes[this->tetra[i].vert[3]]);
					*/
...
void smPhysXVolumeMesh::createEdgeofTetras(){
	...
	for(smInt i=0;i<nbrTetra;i++){
		for(smInt j=0;j<6;j++)
		{
			...
			///edge.vert[0] will be awlays small index
...
//
//void smPhysXVolumeMesh::rotVolumeMesh(smMatrix33<smFloat> p_rot){
//	smVolumeMesh::rotate(p_rot);
//	surfaceMesh->rotate(p_rot);
//
//	
//
//
