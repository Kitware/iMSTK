...
smVBOResult smVBO::updateVertices(smVec3<smFloat> *p_vectors,smVec3<smFloat> *p_normals,smTexCoord *p_textureCoords, smInt p_objectId){
	smInt nbrVertices;
...
smVBOResult smVBO::drawElements(smInt p_objectId){
	 ...
	 //glTexCoordPointer(smGLRealType,smGLRealType ,0, (void*)(dataOffset+nbrVertices*sizeof(smVec3)+nbrVertices*sizeof(smTexCoord)));
...
/// this function must be called when the binding is SOFMIS_VBO_STATIC
 smVBOResult smVBO::initStaticVertices(smVec3<smFloat> *p_vectors,smVec3<smFloat> *p_normals,smTexCoord *p_textureCoords, smInt p_objectId){
	smInt nbrVertices;
	smFloat *baseBufferPtr;
	smFloat *objectBufferPtr;
	smInt dataOffset;
