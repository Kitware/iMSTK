...
void createShader(){
	...
	vs = readStringFromFile("shaders/smokevert.cg");
	fs = readStringFromFile("shaders/smokefrag.cg");
...
void smSpark::createSparkShader(){
	...
	vs = readStringFromFile("shaders/spark_vs.glsl");
	fs = readStringFromFile("shaders/spark_fs.glsl");
...
/// \brief
void SmokeVideo::drawSmokeVideo( bool grapFrame, bool render,float fadeEffect,
                                 float normalizedToolTipX,float normalizedToolTipY,
                                 float depth, float toolX,float toolY,
                                 float toolZ,smDrawParam p_params){
...
/// \brief Properly Closes The Avi File
void SmokeVideo::CloseAVI(void){

}

/// \brief Any User DeInitialization Goes Here
void SmokeVideo::Deinitialize (void){

	frameGrapper->stop();
	frameGrapper->closeAndWait();
}

/// \brief Opens An AVI File (szFile)
void SmokeVideo::OpenAVI2(char* szFile, int textureNumber){
...
/// \brief Any GL Init Code & User Initialiazation Goes Here
bool SmokeVideo::Initialize (){
