...
void transform2ScreenCoord(double x,double y,double z,GLdouble &winX,GLdouble &winY,GLdouble &winZ){
	...
	//cout<<"WINX:"<<winX<<" WINY:"<<winY<<" depth:"<<winZ<<endl;	
...
void smEffectSim::draw(smDrawParam p_params){
	glPushAttrib(GL_ALL_ATTRIB_BITS);
		...
		//depth=left[14]+left[10]*offset;
		depth=dst.z/p_params.rendererObject->camera()->zFar();
		//cout<<"Normalized X:"<<normalizedToolTipX<<" Y:"<<normalizedToolTipY<<endl;
		//normalizedToolTipX=-normalizedToolTipX;
		//normalizedToolTipY=-normalizedToolTipY;
	...
	switch(p_event->eventType.eventTypeCode){
		case SOFMIS_EVENTTYPE_HAPTICOUT:
				...
				if(hapticEventData->deviceId==0){
					...
					//buttonState=hapticEventData->buttonState[0];
