...
public:
    ...
    ///
    /// \brief Return the constraints generated by this handler
    /// This list of constraints is ordered in orderCollisionConstraints
    ///
    const std::vector<PbdConstraint*>& getConstraints() const { return m_collisionConstraints; }
...
private:
    ...
    /// This only works because of the solver we are using (Gauss-Seidel)
    /// TODO: Refactor collision handling to not require ordering so
    /// the constraints can work with other solvers
    ///
    void orderCollisionConstraints();

protected:

    // Enum for binning constraint types to order for solver
    ...
    enum ConstraintType
    {
        ...
        BodyEdge,
        BodyTriangle,
        // mesh on mesh non-ccd constraints
        VertexVertex,
        VertexEdge,
        EdgeEdge,
        VertexTriangle,
